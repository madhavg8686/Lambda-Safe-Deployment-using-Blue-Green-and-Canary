...
variable "use_existing_lambda_execution_role" {
  description = "Use an existing IAM role for Lambda execution instead of creating one."
  type        = bool
  default     = false
}

variable "existing_lambda_execution_role_arn" {
  description = "ARN of an existing IAM role to use for Lambda execution. Required when use_existing_lambda_execution_role is true."
  type        = string
  default     = ""

  validation {
    condition     = var.use_existing_lambda_execution_role ? trim(var.existing_lambda_execution_role_arn) != "" : true
    error_message = "existing_lambda_execution_role_arn must be set when use_existing_lambda_execution_role is true."
  }
}

variable "attach_apigateway_tagging_policy" {
  description = "Attach an existing API Gateway tagging policy to the Terraform IAM user."
  type        = bool
  default     = false
}

variable "existing_apigateway_tagging_policy_arn" {
  description = "ARN of an existing API Gateway tagging IAM policy to attach. Required when attach_apigateway_tagging_policy is true."
  type        = string
  default     = ""

  validation {
    condition     = var.attach_apigateway_tagging_policy ? trim(var.existing_apigateway_tagging_policy_arn) != "" : true
    error_message = "existing_apigateway_tagging_policy_arn must be set when attach_apigateway_tagging_policy is true."
  }
}

variable "terraform_user_name" {
  description = "IAM user name used for Terraform actions"
  type        = string
  default     = "Madhav-terraform"
}
