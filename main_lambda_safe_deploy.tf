...
locals {
  ...
  lambda_execution_role_arn = var.use_existing_lambda_execution_role ? var.existing_lambda_execution_role_arn : aws_iam_role.lambda_execution_role[0].arn
...
resource "aws_iam_role" "lambda_execution_role" {
  count = var.use_existing_lambda_execution_role ? 0 : 1
...
# Attach AWS managed policy for basic Lambda execution only when Terraform creates the role
...
resource "aws_iam_user_policy_attachment" "user_apigateway_tagging" {
  count      = var.attach_apigateway_tagging_policy ? 1 : 0
  user       = var.terraform_user_name
  policy_arn = var.existing_apigateway_tagging_policy_arn
}
